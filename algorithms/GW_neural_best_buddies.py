...
from . import correspondence_loader as Loader
from . import compute_GW as GW

class sparse_semantic_correspondence():
    def __init__(self, model, gpu_ids, tau, border_size, save_dir, k_per_level, k_final, fast, lambda_T):
    ...
    def find_mapping(self, A, B, patch_size, initial_mapping, search_box_radius, mode):
        ...
        for i in range(A.size(2)):
            for j in range(A.size(3)):
                ...
                if mode == "different":
                    index = self.find_closest_patch_index(B_padded, candidate_patch_A, initial_mapping[0, :, i, j],
                                                          search_box_radius)
                else:
                    index = self.find_same_image_closest_patch_index(B_padded, candidate_patch_A,
                                                                     initial_mapping[0, :, i, j],
                                                                     search_box_radius)
    ...
    def find_same_image_closest_patch_index(self, B, patch_A, inital_pixel, search_box_radius):
        [dx, dy] = [math.floor(patch_A.size(2)/2), math.floor(patch_A.size(3)/2)]
        [search_dx, search_dy] = [search_box_radius, search_box_radius]
        up_boundary = int(inital_pixel[0]-search_dx) if inital_pixel[0]-search_dx > 0 else 0
        down_boundary = int(inital_pixel[0]+2*dx+search_dx+1) if inital_pixel[0]+2*dx+search_dx+1 < B.size(2) else B.size(2)
    ...
    def find_neural_best_buddies(self, correspondence, F_A, F_Am, F_Bm, F_B, patch_size, initial_map_a_to_b, initial_map_b_to_a, search_box_radius, tau, top, deepest_level=False, mode='different'):
        ...
        a_to_b = self.find_mapping(F_Am_normalized, F_Bm_normalized, patch_size, initial_map_a_to_b, search_box_radius, mode)
        b_to_a = self.find_mapping(F_Bm_normalized, F_Am_normalized, patch_size, initial_map_b_to_a, search_box_radius, mode)
    ...
    def top_k_in_clusters(self, correspondence, F_A, F_B, k):
        M_A = self.get_M(F_A)
        M_Bt = self.get_M(F_B)
        high_correspondence = [[],[],[]]
        for i in range(len(correspondence[0])):
            ...
            if M_A_i == 1 and M_Bt_i == 1:
                ...
                high_correspondence[1].append(correspondence[1][i])
                high_correspondence[2].append(correspondence[2][i])
        correspondence = high_correspondence
        ...
        for i in range(k):
            max_response = 0
            max_response_index = len(correspondence[0])-1
    ...
    def top_k_final(self, correspondence,  k):
        if k > len(correspondence[0]):
            return correspondence
    ...
    def run(self, A, B):
        ...
        initial_map_a_to_a = self.identity_map(F_A.size())
        initial_map_b_to_b = self.identity_map(F_B.size())
        ...
        for L in range(L_start,self.L_final-1,-1):
            ...
            [correspondence, mapping_a_to_b, mapping_b_to_a] = \
                self.find_neural_best_buddies(correspondence, F_A, F_Am, F_Bm, F_B, patch_size, initial_map_a_to_b,
                                              initial_map_b_to_a, search_box_radius, self.tau, self.k_per_level, deepest_level,
                                              mode='different')
            [correspondence_A, mapping_a_to_a1, mapping_a_to_a2] = \
                self.find_neural_best_buddies(correspondence_A, F_A, F_Am, F_Am, F_A, patch_size, initial_map_a_to_a,
                                              initial_map_a_to_a, search_box_radius, self.tau, self.k_per_level, deepest_level,
                                              mode='same')
            [correspondence_B, mapping_b_to_b1, mapping_b_to_b2] = \
                self.find_neural_best_buddies(correspondence_B, F_B, F_Bm, F_Bm, F_B, patch_size, initial_map_b_to_b,
                                              initial_map_b_to_b, search_box_radius, self.tau, self.k_per_level, deepest_level,
                                              mode='same')

            data_corr = Loader.corrLoader(correspondence_A, correspondence_B, correspondence)

            print("Gromov-Wasserstein Learning Starts...")
            correspondence, trans = GW.GW_correspondence(data_corr, self.lambda_T)
            print("Gromov-Wasserstein Learning Finished...")

            if self.k_per_level < float('inf'):
                correspondence = self.top_k_in_clusters(correspondence, F_A, F_B, int(self.k_per_level))
                correspondence_A = self.top_k_in_clusters(correspondence_A, F_A, F_A, int(self.k_per_level))
                correspondence_B = self.top_k_in_clusters(correspondence_B, F_B, F_B, int(self.k_per_level))
            ...
            [temp1, temp2, temp3, temp4, initial_map_a_to_a, temp] = self.transfer_style_local(F_A, F_A,
                                                                                               patch_size,
                                                                                               image_width,
                                                                                               mapping_a_to_a1,
                                                                                               mapping_a_to_a2,
                                                                                               L)
            [temp1, temp2, temp3, temp4, initial_map_b_to_b, temp] = self.transfer_style_local(F_B, F_B,
                                                                                               patch_size,
                                                                                               image_width,
                                                                                               mapping_b_to_b1,
                                                                                               mapping_b_to_b2,
                                                                                               L)
            [F_A, F_B, F_Am, F_Bm, initial_map_a_to_b, initial_map_b_to_a] = self.transfer_style_local(F_A, F_B,
                                                                                                       patch_size,
                                                                                                       image_width,
                                                                                                       mapping_a_to_b,
                                                                                                       mapping_b_to_a,
        ...
        top_k_correspondence = self.top_k_final(filtered_correspondence, self.k_final)

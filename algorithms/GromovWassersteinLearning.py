...
"""
import copy
from .util2 import logger
import matplotlib.pyplot as plt
import numpy as np
import pickle
from .DataIO import IndexSampler, cost_sampler1, cost_sampler2
...
class GromovWassersteinEmbedding(nn.Module):
    ...
    def __init__(self, num1: int, num2: int, dim: int, cost_type: str = 'cosine', loss_type: str = 'L2'):
        ...
        emb1.weight = nn.Parameter(torch.FloatTensor(self.num1, self.dim).uniform_(-1 / self.dim, 1 / self.dim))
        emb2 = nn.Embedding(self.num2, self.dim)
        emb2.weight = nn.Parameter(torch.FloatTensor(self.num2, self.dim).uniform_(-1 / self.dim, 1 / self.dim))
        self.emb_model = nn.ModuleList([emb1, emb2])

    def orthogonal(self, index, idx, device):
        embs = self.emb_model[idx](index)
        orth = torch.matmul(torch.t(embs), embs)
        orth -= torch.eye(embs.size(1)).to(device)
        return (orth**2).sum()
    ...
    def mutual_cost_mat(self, index1, index2, device):
        embs1 = self.emb_model[0](index1)# (batch_size1, dim)
        embs2 = self.emb_model[1](index2).to(device)  # (batch_size2, dim)
        if self.cost_type == 'cosine':
            # cosine similarity
    ...
    def forward(self, index1, index2, trans, mu_s, mu_t, cost1, cost2, device, prior=None, mask1=None, mask2=None, mask12=None):
        cost_s = self.self_cost_mat(index1, 0)
        cost_t = self.self_cost_mat(index2, 1)
        cost_st = self.mutual_cost_mat(index1, index2, device)
        ...
        regularizer += self.orthogonal(index1, 0, device) + self.orthogonal(index2, 1, device)
...
class GromovWassersteinLearning(object):
    ...
    def evaluation_matching(self, trans: np.ndarray, cost_s: np.ndarray, cost_t: np.ndarray,
                            index_s: np.ndarray, index_t: np.ndarray, mask_s: np.ndarray, mask_t: np.ndarray, device):
        """
        ...
        # edge correctness
        cost_st = self.gwl_model.mutual_cost_mat(index_s, index_t, device)
    ...
    def evaluation_recommendation(self, database, device):
        index_s = torch.LongTensor(list(range(self.src_num))).to(device)
        index_t = torch.LongTensor(list(range(self.tar_num))).to(device)
        # print("evaluation_recommendation",index_t.device,index_s.device)
        cost_st = self.gwl_model.mutual_cost_mat(index_s, index_t, device)
        cost_st = cost_st.cpu().data.numpy()
    ...
    def gromov_wasserstein_discrepancy(self,cost_s, cost_t,  mu_s, mu_t, hyperpara_dict):
        ns = mu_s.size(0)
        nt = mu_t.size(0)
        trans = torch.matmul(mu_s, torch.t(mu_t))
        a = mu_s.sum().repeat(ns, 1)
        ...
        Args:
            ...
            hyperpara_dict: a dictionary of hyperparameters
                dict = {epochs: the number of epochs,
                        ...
                        strategy: hard or soft,
                        beta: the weight of proximal term
                        _iter: the outer iteration of ipot
                        inner_iter: the inner iteration of sinkhorn
                        prior: True or False
                        }
                ...
                for tar_idx, indices2 in enumerate(tar_loader):
                    ...
                    if hyperpara_dict['strategy'] == 'hard':
                        ...
                        if z < epoch/hyperpara_dict['epochs']:
                            ...
                            cost12 = self.gwl_model.mutual_cost_mat(index_s, index_t, device).data
                        else:
                            cost1 = cost_s
                            cost2 = cost_t
                            cost12 = 0
                        ...
                        cost_st_12 = self.gwl_model.mutual_cost_mat(index_s, index_t, device).data
    ...
    def train_with_prior(self, database, optimizer, hyperpara_dict, scheduler=None):
        ...
        for epoch in range(hyperpara_dict['epochs']):
            ...
            for batch_idx, indices in enumerate(dataloader):
                ...
                # self.plot_result(index_s, index_t, epoch, prefix=hyperpara_dict['prefix'])
                # print('src data', database['src_interactions'])
                # print('src index', database['src_index'])
                # print('mmmm', database['mutual_interactions'])
                if hyperpara_dict['strategy'] == 'hard':
                    z = np.random.rand()
                    if z < epoch / hyperpara_dict['epochs']:
                        ...
                        cost12 = mask_st.data * self.gwl_model.mutual_cost_mat(index_s, index_t, device).data
                    else:
                        cost1 = cost_s
                        cost2 = cost_t
                        cost12 = prior
                    ...
                    cost_st_12 = mask_st.data * self.gwl_model.mutual_cost_mat(index_s, index_t, device).data
                ...
                prec, recall, f1 = self.evaluation_recommendation(test_base, device)
    ...
    def save_matching(self, full_path):
        with open(full_path, 'wb') as f:  # Python 3: open(..., 'wb')
            pickle.dump([self.NC1, self.EC1, self.NC2, self.EC2, self.d_gw, self.trans], f)

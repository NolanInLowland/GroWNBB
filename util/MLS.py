...
class MLS():
    ...
    def run_MLS_in_folder2(self, img0_path, img1_path, corrs):
        img_A = Image.open(img0_path).convert('RGB')
        img_B = Image.open(img1_path).convert('RGB')
        img_A = img_A.resize((224, 224), Image.BICUBIC)
        img_B = img_B.resize((224, 224), Image.BICUBIC)
        ...
        for i in range(len(corrs)):
            points_A.append([corrs[i, 0], corrs[i, 1]])
            points_B.append([corrs[i, 2], corrs[i, 3]])
